...
|---|---|---|---|---|
${confidenceData.claims.map(c => `| ${c.orderIndex} | ${c.claimText} | **${c.finalStatus.toUpperCase()}** | **${c.score}%** | \`${c.sourceTitle}\` |`).join('\n')}
`;
...
- **Verdict:** ${c.finalStatus.toUpperCase()} (Score: **${c.score}%**)
- **Substantiating Rationale:** ${c.reasoning}
- **Primary Source Citation:** [${c.sourceTitle}](${c.sourceUrl})
- **Extracted Text Snippet:** *"${c.snippet}"*
`).join('\n')}

## Chapter 3: Risk Assessment & Contradiction Audit
The multi-agent contradiction agent evaluated the claims against empirical benchmarks. Statements regarding rapid timelines were flagged due to deployment delays in current literature.
`;
    } else {
      // EXECUTIVE SUMMARY
      formatSpecificMd = `
...
* **Primary Objective:** Fact-checking and verifying statements regarding "${query}".
* **Overall Session Confidence Score:** **${confidenceData.overallScore}%**
* **Domain Focus:** ${domain} (${depth} Analysis)

### Key Claim Verdicts

${confidenceData.claims.map(c => `* **[${c.finalStatus.toUpperCase()}]** ${c.claimText} — *${c.score}% Confidence score. Source: [${c.sourceTitle}](${c.sourceUrl})*`).join('\n')}
`;
    }
...
${confidenceData.claims.map(c => `- **[${c.sourceTitle}](${c.sourceUrl})**: "${c.snippet}"`).join('\n')}
